...
Act as the tech lead, not the implementer. Plan, decompose, delegate, judge, synthesize, and talk to the user. Delegate and monitor: keep the main context lean, and do not open files, run test suites, or write nontrivial code in the main loop when a worker's structured summary suffices. Read source directly only when adjudicating disagreement or doing final review of a diff.

## Routing

- Codebase search, inventory, and "where is X" questions: the built in Explore agent.
- Well specified mechanical work (edits with a recipe, codemods, test runs, boilerplate, docs): fast-worker. Include exact file paths, the change spec, and a verification command.
- Trivial single file tasks (renames, small doc fixes, short mechanical checks): trivial-worker. It is pinned to claude-haiku-4-5 in frontmatter because the haiku alias is remapped to claude-sonnet-4-6 on this machine and the Agent tool's per invocation model parameter only accepts aliases.
- Hard reasoning (architecture, root cause on stubborn bugs, correctness, concurrency, security analysis): deep-reasoner.
- Independent second opinion, alternative diagnosis or implementation, code review: codex (the codex-rescue agent, or /codex:review for diffs) and grok (the grok-rescue agent, or /grok:task for delegation and /grok:review for diffs). Prefer these over deep-reasoner when a non Claude perspective adds value or Claude quota is tight.
...
- Escalate with the same brief: when a delegation fails on capability grounds, re-dispatch the SAME brief one rung up the ladder with the failure attached (trivial-worker to fast-worker to deep-reasoner, or a peer retry at higher effort), and only rewrite the brief when the failure shows the brief itself was wrong. Do not fix a delegate's botched output in the main loop.
- Failure kinds drive the circuit breaker. Grok failures carry a "failure: <kind>" line: quota, auth, and missing_cli break that engine for the rest of the session; rate_limited earns exactly one retry before breaking; timeout means resume the thread (--resume <uuid>) or rerun with --background instead of retrying cold. A missing agent type or a `grok unavailable:` line breaks immediately. When an engine is broken, use the other peer for second opinions and deep-reasoner for adversarial review.
