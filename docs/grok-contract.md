...
- Failed runs after job creation carry a typed `failureKind` (missing_cli, auth, rate_limited, quota, timeout, permission, error) on the job record plus `state: error` and `failure: <kind>` lines in rendered output. Preflight input errors use `failure: input` without creating a job record. Jobs cancelled by `/grok:cancel`, companion signal forwarding, or the SessionEnd hook are marked `status: cancelled` and `failureKind: cancelled`; rendered output includes `state: cancelled` and `failure: cancelled`. The routing policy parses these lines for session circuit breaking.
...
- The verdict is the first nonempty line in the reply. `BLOCK:` on that first nonempty line blocks the stop; any other first nonempty line allows it. Infrastructure failures always allow.
...
- A bad `--cwd` fails before the companion creates a job record and reports `failure: input`.
