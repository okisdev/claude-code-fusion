...
A `running` status is a claim, not proof of progress. Authoritative companion status, result, and cancel paths verify the driving process and persist lifecycle transitions. Stats observation stays read only, while the Fusion monitor may run the Codex companion's locked repair for a running record whose owner is dead or missing, which can terminate owned processes and finalize the record; Codex status and result still perform the authoritative locked recheck. The Grok observer remains advisory only and never signals a recorded process or rewrites canonical lifecycle state.
...
- `oversized`: A semantic settlement kind recorded via `record-acceptance --failure-kind oversized` when a wind down delivery is only a meta report or empty delta. It indicates a brief sizing error rather than worker judgment.

Engine adapters may define additional typed failure kinds beyond this shared set. The current Codex instance adds `protocol`, for a required structured lifecycle stream that was malformed, incomplete, or incompatible with the adapter, `process`, for an engine process that exited because of an unexpected signal or other unrequested process termination, `policy`, for a forbidden Codex collaboration tool call or a run that resolved to the `persistent` reasoning effort, and `refused`, for a server side policy refusal (misalignment, cyber, or content policy) that Codex delivers only as the turn failure message; the circuit breaker ignores `refused` because it reflects the brief, not lane health. The Grok instance adds `sandbox`, for sandbox initialization or enforcement failure, `transport`, for prompt delivery or structured envelope failure, `policy`, for missing positive tool-policy evidence or a fallback, unmappable, or unmatched tool-policy warning, `turn_limit`, for a write task that exhausted its turn budget with a salvaged partial envelope, and `network`, for a terminal API connectivity failure such as DNS resolution failure, connection refused, or connection reset, which the circuit breaker treats as a repeated class that opens on the second consecutive occurrence.
