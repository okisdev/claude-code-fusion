...
The companion checks the installed Codex version during setup and records it on every job when available. Preflight hard-fails with `failureKind: "setup"` when the installed Codex CLI version parses below the tested minimum 0.146.0 and tells the user to upgrade. The tested interval runs from 0.146.0 up to but excluding 0.147.0; versions at or above 0.147.0 remain allowed with the setup compatibility advisory. Live smoke verification against 0.146.0 on 2026-07-29 confirmed task execution, thread resume, task mode structured output, JSONL rollout persistence under `~/.codex/sessions`, and native review target flags all matched this contract. Setup also verifies CLI authentication and a writable adapter data directory. A nonempty `CODEX_API_KEY` is accepted as exec authentication when `codex login status` reports logged out because that probe does not inspect environment based authentication. Unknown JSONL event types are retained and ignored unless they affect terminal correctness. Missing or changed required lifecycle events fail closed as protocol errors.
