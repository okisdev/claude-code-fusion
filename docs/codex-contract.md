...
At 0.147.0, the features registry defined `multi_agent` (enabled by default) and `multi_agent_v2`, so the companion's `--disable` pair remained valid and load-bearing. Re-verified at 0.152.0, the registry still defines both features. `codex exec` still starts only an in-process app-server client: `app-server`, `exec-server`, `remote-control`, and `cloud` remain explicit subcommands, never implicit daemons. The session rollout layout remains `sessions/YYYY/MM/DD/rollout-<timestamp>-<uuid>.jsonl` with `archived_sessions`; upstream removed `codex exec --full-auto`, which the companion never used. Exec resume still passes `exclude_turns` upstream without changing the observable envelope.
...
The companion checks the installed Codex version during setup and records it on every job when available. Preflight hard-fails with `failureKind: "setup"` when the installed Codex CLI version parses below the tested minimum 0.147.0 and tells the user to upgrade. The tested interval runs from 0.147.0 up to but excluding 0.153.0; versions below 0.147.0 fail task and review preflight with a setup error; versions at or above 0.153.0 keep running task and review preflight while `/codex:setup` reports not ready with exit code 1 as the compatibility advisory. Live smoke verification against codex-cli 0.152.0 on 2026-09-01 confirmed task execution, thread resume by stored thread id, structured output via `--output-schema`, and JSONL rollout persistence. Setup also verifies CLI authentication and a writable adapter data directory. A nonempty `CODEX_API_KEY` is accepted as exec authentication when `codex login status` reports logged out because that probe does not inspect environment based authentication. Unknown JSONL event types are retained and ignored unless they affect terminal correctness. Missing or changed required lifecycle events fail closed as protocol errors.
