...
A `running` record is not proof of liveness. Companion status, result, and cancel paths verify the companion and Codex PIDs. A record remains running during a short pidless launch grace period. After that period, a companion read path atomically finalizes a record without a live owner as `error` with `failureKind: "died"`. Stats observation stays read only, while the Fusion monitor may run the companion's locked repair for a running record whose owner is dead or missing, which can terminate owned processes and finalize the record; Codex status and result still perform the authoritative locked recheck. The explicit `/fusion:stats --prune-dead --yes` cleanup remains a separate user-authorized deletion path.
...
Every Codex exec invocation disables `multi_agent`, `multi_agent_v2`, `sleep_tool`, and `memories`. The sleep tool disable exists because GPT-6 Astra advertises the `clock` tool and Codex then registers a sleep tool under `codex exec` that would spend the foreground flight budget waiting. The memories disable keeps a delegated run out of the account's cross thread memory store, so a brief from one repository can neither feed nor read memories that later runs in another repository would see. Both features are present in the 0.152.0 registry and `sleep_tool` is absent at 0.147.0, which is why the tested minimum moves to 0.152.0. The adapter also fails the job with `failureKind: "policy"` and `semanticStatus: "rejected"` if the event stream or matching rollout still shows a collaboration tool call, or if the matching rollout shows that the run resolved to the `persistent` reasoning effort through a configuration layer the preflight check cannot see. Delegated Codex therefore remains one owned, bounded engine execution instead of recursively creating Codex agents or drifting into proactive follow up work.

Guardian approval reviews cannot run inside a companion job. Upstream routes an action to Guardian only under the on request or granular approval policies with the auto review reviewer, and the companion pins `approval_policy="never"`, so the hidden `codex-auto-review` catalog model and the `node_repl_auto_review_required` flag that GPT-6 Astra alone carries have no effect on delegated runs (guardian routing read at rust-v0.153.4).

Skills and plugins stay under the user's Codex configuration. The companion passes neither `skills.include_instructions` nor plugin toggles, so every delegated run pays the context cost of the enabled skill catalog, and when Codex trims that catalog it reports `Skill descriptions were shortened to fit the skills context budget` as an `item.error` diagnostic that the job record retains. The remedy is user side: the `skills.max_context_tokens` key or disabling unused plugins in `config.toml`.

At 0.147.0, the features registry defined `multi_agent` (enabled by default) and `multi_agent_v2`, so the companion's `--disable` pair remained valid and load-bearing. Re-verified at 0.152.0, 0.153.2, and 0.153.4, the registry still defines both features, and 0.153.4 lists `sleep_tool` and `memories` as stable enabled features. `codex exec` still starts only an in-process app-server client: `app-server`, `exec-server`, `remote-control`, and `cloud` remain explicit subcommands, never implicit daemons. The session rollout layout remains `sessions/YYYY/MM/DD/rollout-<timestamp>-<uuid>.jsonl` with `archived_sessions`; upstream removed `codex exec --full-auto`, which the companion never used. Exec resume still passes `exclude_turns` upstream without changing the observable envelope.
...
Model and reasoning effort remain unset unless explicitly requested. The Codex configuration remains authoritative for defaults. The companion explicitly sets web search to disabled and workspace-write network access to false by default. `--web` opts a read-only or write task into live Codex web search. `--network` requires `--write` and opts the workspace-write sandbox into network access. Task mode `--output-schema <path>` validates a regular JSON Schema file up to 256 KiB and parses the final agent message once, while the Codex CLI `review` subcommand accepts but silently ignores `--output-schema`, unchanged from 0.144.6 through the live-verified 0.147.0 and re-verified at 0.152.0.

The `persistent` reasoning effort is rejected before job creation with `failureKind: "input"`. It switches Codex into the open ended persistent mode that GPT-6 Astra ships developer instructions for, and a bounded companion flight cannot host proactive follow up work. The preflight check covers an explicit `--effort` and an effort inherited through a resumed thread. A `persistent` default that reaches the run through `config.toml`, a profile, or a command line override is caught after the fact instead: the matching rollout's `turn_context` reports the effective effort, and a run that resolved to `persistent` terminalizes as `failureKind: "policy"` with `semanticStatus: "rejected"`. The companion does not parse Codex configuration to predict that outcome, because a partial parse misreads profile overrides and multiline strings; the spent flight is the price of that boundary, and `/fusion:doctor` flags the configured default before it costs one. Every other effort value the catalog lists for the requested model passes through unchanged.

A job whose resolved model differs from its explicit `--model` request records `modelDrift` with `requestedModel` and `resolvedModel` and renders a warning line on the terminal output and on `result`. Codex substitutes the model when an account falls into Luna Reserve, whose hidden `gpt-reserve` model replaces the requested one until the advanced model allowance returns. A task whose brief header names a model while the request leaves `--model` unset keeps recording the existing header drift shape.
...
The companion checks the installed Codex version during setup and records it on every job when available. Preflight hard-fails with `failureKind: "setup"` when the installed Codex CLI version parses below the tested minimum 0.152.0 and tells the user to upgrade. The tested interval runs from 0.152.0 up to but excluding 0.154.0; versions below 0.152.0 fail task and review preflight with a setup error; versions at or above 0.154.0 keep running task and review preflight while `/codex:setup` reports not ready with exit code 1 as the compatibility advisory. Live verification on 2026-09-05 confirmed task execution (including a live web search consult on gpt-6-astra at 0.153.2), thread resume by stored thread id, structured output via `--output-schema`, native review, and JSONL rollout persistence on codex-cli 0.153.2 and again on 0.153.4. Setup also verifies CLI authentication and a writable adapter data directory. A nonempty `CODEX_API_KEY` is accepted as exec authentication when `codex login status` reports logged out because that probe does not inspect environment based authentication. Unknown JSONL event types are retained and ignored unless they affect terminal correctness. Missing or changed required lifecycle events fail closed as protocol errors.
...
Codex records are the primary source for transport status, delivery, semantic status, model request, resolved model and effort, token usage, thread identity, Claude session identity, repository identity, and job duration. Without a state override, Fusion reads only the canonical `codex-claude-code-fusion/state` root. Fusion reads only that canonical root or the single `FUSION_CODEX_STATE` override, and the legacy `codex-openai-codex` root is never consulted. The new `/codex:result` command cannot collect an old plugin deliverable. `CODEX_COMPANION_DATA` selects only its own state root, just as the Fusion state overrides select only one root. Fusion observes those records without taking ownership of their lifecycle. A `done` Codex job remains `unverified` until the orchestrator checks its completion criteria or verification command and records `accepted` or `rejected`. `/fusion:stats --record` writes the verdict into the Codex job record through the companion's `record-acceptance` subcommand. Marking a job accepted after transport failure requires the explicit `--accept-failed-transport` override, and the stats report exposes acceptance anomalies.

`record-acceptance` requires `--reason` for a rejected verdict and accepts `--failure-kind` with one of `intent_override`, `scope_rewrite`, `wrong_approach`, `style_mismatch`, or `oversized` to classify judgment shaped rejections; the kind lands in `semanticFailureKind`.
