...
At 0.147.0, the features registry still defines `multi_agent` (enabled by default) and `multi_agent_v2`, so the companion's `--disable` pair remains valid and load-bearing. `codex exec` starts only an in-process app-server client: `app-server`, `exec-server`, `remote-control`, and `cloud` are explicit subcommands, never implicit daemons. The session rollout layout remains `sessions/YYYY/MM/DD/rollout-<timestamp>-<uuid>.jsonl` with `archived_sessions`; upstream removed `codex exec --full-auto`, which the companion never used. Exec resume now passes `exclude_turns` upstream without changing the observable envelope.
