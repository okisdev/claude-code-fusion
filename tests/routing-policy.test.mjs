...
test("Claude workers stay inside their advisory and fallback roles", () => {
  ...
  assert.equal(frontmatterValue(deepReasoner, "background"), "true");
  ...
  assert.equal(frontmatterValue(fastWorker, "background"), "true");
  ...
  assert.equal(frontmatterValue(trivialWorker, "background"), "true");
...
test("Grok automatic routing requires exactly one of five protected roles", () => {
  ...
  assert.match(grokRescue, /`phase: cleanup-required` is a nonterminal failure receipt that must be returned instead/);
  assert.match(grokRescue, /A cleanup-required receipt is the only valid managed final whose recorded state remains running\./);
  ...
  assert.equal(frontmatterValue(grokRescue, "background"), "true");
});
...
test("the panel stays blind, asymmetric, evidence weighted, and main judged", () => {
  ...
  assert.match(panel, /start `grok:grok-rescue` as a background Agent and invoke `codex:codex-rescue` as a foreground Agent, each with the identical brief as its prompt/);
  ...
  assert.match(panel, /save both exact derived brief texts outside the repository before launching them\./);
