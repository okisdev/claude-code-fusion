...
Multi-model orchestration for Claude Code. The strongest available Claude model runs your session as a tech lead, cheaper Claude models do the execution, and peer engine CLIs can act as implementation and review lanes whose work bills to their own subscriptions. Today those peers are Codex through the external `codex@openai-codex` plugin and Grok through the hosted `grok` plugin in this marketplace; the companion contract is engine agnostic so more lanes can fit the same shape. The orchestrator and the Claude worker tiers still bill to your Claude plan; delegation moves the heavy execution off it.
...
Enforced by companion runtimes (code, covered by the test suite): consult runs are constrained read oriented delegations with workspace sandboxing where the engine supports it, explicit write denies, and nested engine launch denies. Write mode allows repository edits and uses a short deny list for dangerous commands; that list is a minimal exception set, not a sandbox, so run write delegations on a clean branch or a disposable worktree. The stop gate is an optional companion review before SessionEnd and fails open on infrastructure errors. Grok's current instance uses `--sandbox workspace`, `dontAsk` for consult, `--always-approve` for write and best-of-n, explicit denies for Edit, Write, metacharacters, nested `grok`, `claude`, `codex`, and `node` launches in consult, and sudo, `rm -rf`, `git push`, and nested engine launches in write; its permission matcher notes are in [docs/grok-contract.md](docs/grok-contract.md), and the shared outcome rules are in [docs/companion-contract.md](docs/companion-contract.md).

Requested by prompts (the orchestrator follows the installed instructions; not runtime enforced): the routing policy, the panel's blindness, review never applying fixes, and the session circuit breaker that stops routing to a peer after its unavailable prefix or shared failure kinds such as `quota`, `auth`, or `missing_cli`. Each engine can define its own unavailable prefix; the one present today is `grok unavailable:`. `/fusion:panel` substitutes `fusion:deep-reasoner` for a missing track rather than aborting.
