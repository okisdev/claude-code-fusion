...
| Peer engines | companion plugins implementing the shared contract. Current instances are Grok CLI (the hosted `grok` plugin here) and Codex CLI (the external `codex@openai-codex` plugin) | Codex is the primary implementation lane on the current GPT 5.x flagship and expects spec grade briefs (explicit completion criteria, output contract, boundaries, verification command); Grok rides the grok CLI's flagship default (Grok 4.5) and stays fast, taking implementation packages as well as small fixes, drafts, research digests, and large context reads. Trivial single file tasks default to Grok at low effort |
