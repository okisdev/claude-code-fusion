...
Multi-model orchestration for Claude Code. Your chosen Claude session model acts as a tech lead, cheaper Claude models do the execution, and peer engine CLIs can act as implementation and review lanes whose work bills to their own subscriptions. Today those peers are Codex through the external `codex@openai-codex` plugin and Grok through the hosted `grok` plugin in this marketplace; the companion contract is engine agnostic so more lanes can fit the same shape. The orchestrator and the Claude worker tiers still bill to your Claude plan; delegation moves the heavy execution off it. The plugin audits but never sets the session model.
...
| Claude workers | `fusion:deep-reasoner` (Fable), `fusion:fast-worker` (Sonnet), `fusion:trivial-worker` (Haiku) | deep reasoning, mechanical work, trivial tasks when the Codex lane is unavailable |
| Peer engines | companion plugins implementing the shared contract. Current instances are Grok CLI (the hosted `grok` plugin here) and Codex CLI (the external `codex@openai-codex` plugin) | Codex is the primary implementation lane and expects spec grade briefs (explicit completion criteria, output contract, boundaries, verification command). Quick scoped packages default to `gpt-5.6-terra` at xhigh effort; trivial and high volume light work default to `gpt-5.6-luna` at xhigh effort. Grok holds reserved seats for same turn overflow while the Codex lane is occupied, independent second opinions and the panel cross check, live web research consults, very large context reads, and best of n tournaments. Each lane runs whatever its own CLI is configured for; model names and scores live in the /fusion:config capability table, refreshed from each CLI's live listing. |
| Panel | `/fusion:panel` | one blind brief to available peer engines in parallel, adjudicated with attribution |

## Quick start

Prerequisites: Claude Code 2.1.198 or later (the dispatch flow relies on background subagents being the default) and Node.js 22 or later (tested on 2.1.198 and Node 24; the floors are not enforced), git, macOS or Linux, and xAI's grok CLI installed and authenticated per xAI's grok CLI documentation, on PATH, verified with `grok --version`. The Codex side powers the primary implementation lane and default scoped lanes; install the `codex` plugin from [openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc) (`codex@openai-codex`) and run `/codex:setup` once. Without it fusion still runs, with spec grade packages falling back to the Claude workers and scoped packages to Grok, which also keeps its reserved seats.
...
| `/grok:review [--base <ref>] [--focus <text>]` | Read only adversarial review of the working tree or a branch range (untracked files reach the review as names only); the review run never edits, and acting on confirmed findings is a separate orchestrator decision after it returns |
...
| `/fusion:stats [--all]` | Delegation history across peer engines |
...
| `/fusion:ultra <task>` | Fans a large task out as a fleet of parallel peer engine agents billed to their own subscriptions, then synthesizes. Fires from "go deep", "audit everything", "be exhaustive"; skips small tasks |
...
Companion outcomes use the shared footer grammar for `state`, `failure`, `job`, and `<engine>-session` lines defined in [docs/companion-contract.md](docs/companion-contract.md). Each plugin names its own session line, for example Grok emits `grok-session: <uuid>`.

## Safety model

Two layers with different strength, stated plainly.

Enforced by companion runtimes (code, covered by the test suite): consult runs are read only delegations with workspace sandboxing where the engine supports it, explicit write denies, and nested engine launch denies. Their narrow allow list includes `node --test`, `npm test`, and read only `git` and `gh`; they cannot edit or launch nested engine CLIs. Write mode allows repository edits and uses a short deny list for dangerous commands; that list is a minimal exception set, not a sandbox, so run write delegations on a clean branch or a disposable worktree. The optional stop gate runs from the Stop hook, not SessionEnd, and parses its first nonempty reply line: `ALLOW` allows the stop, while `BLOCK: <reason>` blocks it. A `BLOCK` after a preamble and every infrastructure failure fail open. SessionEnd runs job cleanup. Grok's current instance uses `--sandbox workspace`, `dontAsk` for consult, `--always-approve` for write and best-of-n, explicit denies for Edit, Write, metacharacters, nested `grok`, `claude`, `codex`, and `node` launches in consult, and sudo, `rm -rf`, `git push`, and nested engine launches in write; its permission matcher notes are in [docs/grok-contract.md](docs/grok-contract.md), and the shared outcome rules are in [docs/companion-contract.md](docs/companion-contract.md).
...
The grok plugin keeps job records, briefs, and logs under `~/.claude/plugins/data/grok-claude-code-fusion/`; briefs can contain your prompts and diffs, and logs can contain grok stderr. The fusion plugin keeps model routing and reconciliation data under `~/.claude/plugins/data/fusion-claude-code-fusion/`: `model-routing.json`, `reconcile-decisions.json`, `inline-guard/`, and `observations/`. Delete those directories to clear history. Full uninstall: remove both plugins via `/plugin`, delete `~/.claude/rules/orchestration.md`, drop the optional `Bash(node:*)` entry from `permissions.allow`, and delete both data roots. Environment overrides (`GROK_BIN`, `GROK_COMPANION_DATA`, `GROK_COMPANION_TIMEOUT_MS`, `GROK_COMPANION_PIDLESS_RUNNING_GRACE_MS`, `GROK_COMPANION_WAIT_POLL_MS`, `GROK_COMPANION_WAIT_TIMEOUT_MS`, `GROK_CONSULT_ALLOW`) are documented in [docs/grok-contract.md](docs/grok-contract.md). `/fusion:stats` aggregates delegation counts across both peers; token usage itself lives with each vendor (ccusage for the Claude side, the OpenAI and xAI dashboards for the peers), since peer work never touches the Claude transcript.
