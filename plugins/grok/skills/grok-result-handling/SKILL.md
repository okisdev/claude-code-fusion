...
- After presenting review findings, triage them by verification, not by asking. Verify each finding against the current code: dispatch fixes for the confirmed ones without prompting, and drop false positives with a stated reason. Only a finding that forces a genuine product or design decision goes to the user as a question. Close with one report: what was fixed, what was dropped, and why.
