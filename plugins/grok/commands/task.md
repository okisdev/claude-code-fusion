...
argument-hint: '[--write] [--background] [--resume <uuid>|--resume-last] [--model <id>] [--effort <level>] [--max-turns <n>] [--best-of-n <n>] [what Grok should do]'
...
- Separate routing flags from the task text: `--write`, `--background`, `--resume <uuid>`, `--resume-last`, `--model`, `--effort`, `--max-turns`, and `--best-of-n` pass through as flags, and the remaining natural language text passes as one quoted positional prompt.
