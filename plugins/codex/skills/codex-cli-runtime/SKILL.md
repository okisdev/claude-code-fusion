...
- Every companion invocation disables the Codex `multi_agent`, `multi_agent_v2`, `sleep_tool`, `memories`, and `goals` features. A collaboration tool event, or a rollout that resolved to the `persistent` reasoning effort, fails the job as `policy` even if an upstream configuration bypasses those flags.
