...
| Quick scoped package: edits with a recipe, codemods, small fixes across a few files | grok implementation lane | The brief names low effort, or the account's fast coding SKU from the live model listing, explicitly, because the lane default is the CLI's current flagship rather than a cheap tier. Drafts and research digests also land here |
