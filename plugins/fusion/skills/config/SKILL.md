...
3. Propose a scorecard when the user asked for a proposal (for example invoked this skill with "propose") or when the capability table from step 2 has no scored models.
   - Build one proposed row per engine from the live state gathered in step 1: the grok CLI's current default from `grok models`, the codex default from the extracted `~/.codex/config.toml` keys, and the three Claude worker pins. Model ids come only from that live enumeration, never from memory or a hardcoded list. Skip any engine that came back unavailable.
