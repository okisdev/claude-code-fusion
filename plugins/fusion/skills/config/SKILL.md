...
allowed-tools: Read, Glob, Edit, Bash(grok models:*), Bash(rg:*), Bash(node:*), AskUserQuestion, WebSearch, WebFetch
---
...
3. Propose a scorecard when the user asked for a proposal (for example invoked this skill with "propose") or when the capability table from step 2 has no scored models.
   ...
   - Apply only the confirmed rows with the existing rescore command from step 4, one invocation per model with `--lane` and `--notes`, then run `show` and display the resulting table.
4. Offer changes with the `AskUserQuestion` tool, then apply only what the user picked.
   ...
   - For Codex, edit `~/.codex/config.toml` only when the user chooses a Codex default or reasoning effort change, touch only the `model` and `model_reasoning_effort` keys, and preserve the rest of the file byte for byte. Effort values are per model rather than one fixed list; read the current model's supported efforts from the live catalog (`codex debug models --bundled` or a scoped extraction of `~/.codex/models_cache.json`) before writing one, since the installed codex plugin wrapper's `codex-companion.mjs` can accept a narrower set of effort values than the CLI catalog exposes for that model, and a config file default is the only way to reach a tier the wrapper rejects; the config file holds defaults, and per call --model and --effort overrides on the codex companion take precedence for a single job.
