...
4. Report what was installed, what was skipped, and remind the user that future plugin updates sync automatically at the next session start unless the live copy carries local edits, in which case this command is how to reconcile them. When the live rules still carry the shipped placeholder model table, point the user at /fusion:config propose as the next step.
